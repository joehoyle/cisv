...
class WidgetCache
{
	...
	var $plugin_uri='http://www.nexwp.com';
	...
	function WidgetCache()
	{
		...
		if($_SERVER['HTTP_HOST'])$this->cachedir.='/'.$_SERVER['HTTP_HOST'];

		$this->wcache = new WCache($this->cachedir);
		if(!(is_dir($this->cachedir)&&is_writable($this->cachedir)))
		{
			add_action('admin_notices', array(&$this,'widget_cache_warning'));
		}
		else
		{
			$this->wgc_load_opts();
			...
			if($this->wgcEnabled)
			{
				if($this->wgcAutoExpireEnabled)
				{
					...
					"category" => array("create_category","edit_category","delete_category"),
					"comment" => array("comment_post","edit_comment","delete_comment","pingback_post","trackback_post","wp_set_comment_status"),
					"link" => array("add_link","edit_link","delete_link"),
					"post" => array("delete_post","save_post"),
					"tag" => array("create_term","edit_term", "delete_term"),
					);
				}
				if($this->wgcVaryParamsEnabled)
				{
					$this->varyParams=array(
					"userLevel" => array(&$this,'get_user_level'),
					"userAgent"=>array(&$this,'get_user_agent'),
					"currentCategory"=>array(&$this,'get_current_category'),
					);
				}
				add_action('wp_head', array(&$this,'widget_cache_redirect_callback'), 99999);
			}
			...
			if(is_admin())
			{
				...
				if($this->wgcEnabled)add_action('sidebar_admin_page', array(&$this,'widget_cache_options_filter'));
				add_action('sidebar_admin_setup', array(&$this, 'widget_cache_expand_control'));
				if($_GET["wgdel"])
				{
					add_action('admin_notices', array(&$this,'widget_wgdel_notice'));
				}
			}
		}
	}
	...
	function wp_add_options_page()
	{
		...
		{
			add_options_page( $this->plugin_name, $this->plugin_name, 8, basename(__FILE__), array(&$this,'wp_options_subpanel'));
		}
	}

	function wp_options_subpanel()
	{
		if($_POST["widget_cache-clear"] || $_GET['clear']=="1")
		{
			$this->wcache->clear();
			echo '<div id="message" class="updated fade"><p>Cache Cleared</p></div>';
		}

		if($_POST["wp_wgc_submit"])
		{
			$wp_settings = array (
		...
		else if($_POST["wp_wgc_load_default"])
		{
		...
		<div class="wrap">
		  <form action="<?php echo $_SERVER['PHP_SELF']; ?>?page=widget-cache.php" method="post">
		    ...
		    <table class="form-table">
		      <tr valign="top">
		        <td>
		        <input name="wgc_enabled" type="checkbox" id="wgc_enabled" value="1" <?php checked('1', !($wp_settings["wgc_disabled"]==="1")); ?> />
		        <label for=wgc_enabled><strong>Enable Widget Cache</strong> </label>
		        </td>
		      </tr>
		      <tr valign="top">
		        <td>
		        <input name="wgc_ae_ops_enabled" type="checkbox" id="wgc_ae_ops_enabled" value="1" <?php checked('1', !($wp_settings["wgc_ae_ops_disabled"]==="1")); ?> />
		        <label for=wgc_ae_ops_enabled><strong>Enable auto expire options (e.g. When categories, comments, posts, tags changed)</strong> </label>
		        </td>
		      </tr>
		      <tr valign="top">
		        <td>
		        <input name="wgc_vary_by_params_enabled" type="checkbox" id="wgc_vary_by_params_enabled" value="1" <?php checked('1', ($wp_settings["wgc_vary_by_params_enabled"]=="1")); ?> />
		    ...
		    <p class="submit">
		      ...
		      <input type="submit" name="wp_wgc_submit" value="Save Options &raquo;" class="button" style="margin-left:15px;" />
		    </p>
		    <p>
	...
	function widget_cache_options_filter()
	{
		...
		<div class="wrap">
			<form method="POST">
				...
				<span class="submit">					
					<input type="submit" name="widget_cache-clear" id="widget_cache-options-submit" value="Clear all widgets cache(<?php echo $this->wcache->cachecount(); ?>)" />
				</span>
				</p>
			</form>
		</div>
	...
	function widget_cache_expand_control()
	{
		...
		if ($this->wgcEnabled)
		{
			foreach ( $wp_registered_widgets as $id => $widget )
			{
				...
				wp_register_widget_control($id,$widget['name'], array(&$this, 'widget_cache_empty_control'));
				...
				$wp_registered_widget_controls[$id]['callback']=array(&$this, 'widget_cache_extra_control');
			}
			...
			if ( 'post' == strtolower($_SERVER['REQUEST_METHOD']) )
			{
				...
				{
					if (isset($_POST[$widget_id.'-widget_cache']))
					{
						$wc_options[$widget_id]= intval($_POST[$widget_id.'-widget_cache']);
					}
					if($this->wgcAutoExpireEnabled)
					{
						if(isset($_POST[$widget_id.'-widget_cache-trigger']))
						{
							$wc_trigers[$widget_id]=($_POST[$widget_id.'-widget_cache-trigger']);
						}
						else
						{
				...
				foreach (array_keys($wc_options) as $key)
				{
					if (!in_array($key, $regd_plus_new))
					{
						...
						if($this->wgcAutoExpireEnabled)unset($wc_trigers[$key]);
						if($this->wgcVaryParamsEnabled)unset($wc_varyparams[$key]);
					}
				}
	
				$this->wgc_update_option('widget_cache', $wc_options);
				
				if($this->wgcAutoExpireEnabled)$this->wgc_update_option('widget_cache_action_trigger', $wc_trigers);
				if($this->wgcVaryParamsEnabled)$this->wgc_update_option('widget_cache_vary_param', $wc_varyparams);
	...
	function widget_cache_extra_control()
	{
		...
		{
			$wc_options = $this->wgcOptions;
			$value=htmlspecialchars(stripslashes($wc_options[$id]),ENT_QUOTES);
			...
			<fieldset style="border:1px solid #2583AD; padding:3px 0 3px 5px; margin-bottom:10px; line-height:150%;">
				...
				<div>		
					Expire in <input type='text' 
					name='<?php echo $id_disp; ?>-widget_cache' 
					id='<?php echo $id_disp; ?>-widget_cache' value='<?php echo $value; ?>' size=6 style="padding:0" /> 
					second(s) (<a href='widgets.php?wgdel=<?php echo urlencode($id_disp) ?>'>Delete</a>)
					<br/>(Left empty means no cache)
				</div>
				<?php
				if($this->wgcAutoExpireEnabled)
				{
					...
					<div style="margin-top:5px; border-top:1px solid #ccc; clear:both;">
						...
						<?php
						foreach ($this->triggerActions as $tkey=>$actArr)
						{
							$checked="";
							if($this->wgcTriggers[$id_disp] && in_array($tkey, $this->wgcTriggers[$id_disp]))
							{
								$checked="checked=\"checked\"";
							}
						?>
						<div style="float:left; display:inline; margin:2px 1px 1px 0;" nowrap="nowrap">
							<input type="checkbox" <?php echo $checked;?>  id="<?php echo $id_disp; ?>-widget_cache-trigger-<?php echo $tkey;?>" name="<?php echo $id_disp; ?>-widget_cache-trigger[]" value="<?php echo $tkey;?>" />
							<label for="<?php echo $id_disp; ?>-widget_cache-trigger-<?php echo $tkey;?>"><?php echo ucwords($tkey);?></label>
						</div>
						<?php
						}
						?>			
				...
				if($this->wgcVaryParamsEnabled)
				{
					...
					<div style="margin-top:5px; border-top:1px solid #ccc; clear:both;">
						Vary by:
						<?php
						foreach ($this->varyParams as $vparam => $vfunc)
						{
							$checked="";
							if($this->wgcVaryParams[$id_disp] && in_array($vparam, $this->wgcVaryParams[$id_disp]))
							{
								$checked="checked=\"checked\"";
							}
						?>
						<div style="float:left; display:inline; margin:2px 1px 1px 0;" nowrap="nowrap">
							<input type="checkbox" <?php echo $checked;?>  id="<?php echo $id_disp; ?>-widget_cache-varyparam-<?php echo $vparam;?>" name="<?php echo $id_disp; ?>-widget_cache-varyparam[]" value="<?php echo $vparam;?>" />
							<label for="<?php echo $id_disp; ?>-widget_cache-varyparam-<?php echo $vparam;?>"><?php echo ucwords($vparam);?></label>
						</div>
						<?php
						}
						?>			
		...
		{
			?>
			<label style='color:gray; font-style:italic; margin-bottom:10px; line-height:150%;'>WP Widget Cache disabled</label>
			<?php
		}
	...
	function widget_cache_redirect_callback()
	{
		...
		foreach ( $wp_registered_widgets as $id => $widget )
		{
			...
			$wp_registered_widgets[$id]['callback']=array(&$this, 'widget_cache_redirected_callback');
		}
	}
...
function widget_cache_hook_trigger()
{
	...
	if(sizeof($widget_cache->wgcTriggers))
	{
		foreach ($widget_cache->wgcTriggers as $wgid=>$wgacts)
		{
			...
			{
				if($widget_cache->triggerActions[$wact])
				{
					foreach ($widget_cache->triggerActions[$wact] as $wpaction)
					{
						add_action($wpaction, create_function('$actionarg','widget_cache_remove("'.addslashes($wgid).'");'), 10, 1);
					}
				}
			}
		}
...
if($widget_cache->wgcAutoExpireEnabled)widget_cache_hook_trigger();
?>